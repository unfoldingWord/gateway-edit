...
export function flattenChecks(quotesIndices) {
  let checks = []
  ...
  return { checks, missingWord }
}

export async function findQuoteMatches(bookID, chapter, verse, quote) {
  console.log(`findQuoteMatches - quote`,quote)
  ...
  try {
    ...
    const { checks, missingWord } = flattenChecks(quotesIndices)

    if (checks?.length) {
      const { sortedChecks, sortedtWords } = getSortedListOfChecks(checks)
      console.log(`findQuoteMatches - sortedtWords`,sortedtWords)

      if (sortedChecks?.length) {
        sortedChecks_ = sortedChecks_.concat(sortedChecks)
        sortedTWords_ = sortedTWords_.concat(sortedtWords)
      }
    }
    ...
    if (missingWord || !(checks?.length > 4)) {
      ...
      if (bookObject) {
        ...
        for (const quote of Object.keys(quotesIndices)) {
          ...
          const strong = vo?.strong

          if (strong) {
            strongWords.push(strong)
          }
        }

        console.log(`findQuoteMatches - strongWords`, strongWords)
        const strongsChecks = await getChecksForWords('strongsIndex', testament, strongWords)
        console.log(`findQuoteMatches - strongsChecks`, strongsChecks)
        let { checks } = flattenChecks(strongsChecks)

        if (checks?.length) {
          const { sortedChecks, sortedtWords } = getSortedListOfChecks(checks)
          console.log(`findQuoteMatches - sortedtWords`,sortedtWords)

          for (let i = 0; i < sortedtWords?.length; i++) {
            const tWord = sortedtWords[i]

            if (!sortedTWords_.includes(tWord)) {
              sortedTWords_.push(tWord)
              sortedChecks_.push(sortedChecks[i])
            }
          }
        }
      }
...
function findWordInVerse(bookObject, chapter, verse, quote) {
  if (quote && bookObject) {
    const verseObjects = bookObject.chapters?.[chapter]?.[verse]?.verseObjects || []
...
export async function getChecksForWords(indexKey, testament, wordsList) {
  ...
  if (wordsIndex) {
    for (const word of wordsList) {
      checks_ = null
      const checksToLookUp_ = wordsIndex[word] || wordsIndex[word.toLowerCase()]
...
export function getSortedListOfChecks(checks) {
  ...
  const sortedtWords = Object.keys(tWords).sort((a, b) => (-tWords[a]?.length + tWords[b]?.length))
  const sortedChecks = sortedtWords.map(tWord => tWords[tWord])
